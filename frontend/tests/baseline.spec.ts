...
test("Capture Visual Baselines for English and Arabic", async ({ page }) => {
  ...
  for (const locale of locales) {
    ...
    // Locate primary sections on the home page
    const sections = [
      { name: "01_hero", selector: 'section[aria-label="Manifesto"]' },
      { name: "02_showcase", selector: 'section[aria-label="Aivora Product Demo"]' },
      { name: "03_services", selector: "section#services, section:has-text('Services')" },
      { name: "04_industries", selector: "section:has-text('Industries')" },
      { name: "05_why_aivora", selector: "section:has-text('Why Aivora')" },
      { name: "06_process", selector: "section:has-text('Process')" },
      { name: "07_team", selector: "section:has-text('Team')" },
      { name: "08_faq", selector: "section:has-text('FAQ')" },
      { name: "09_contact", selector: "section#contact, form" },
