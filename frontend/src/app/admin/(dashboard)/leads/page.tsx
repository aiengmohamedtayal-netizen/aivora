  ...
  const fetchLeads = async () => {
    ...
    try {
      ...
      if (error || !data || data.length === 0) {
        // Mock data fallback if database is empty/unconfigured
        setLeads([
          { 
            id: "1", 
            name: "Ahmed", 
            ...
            company: "Riyadh Tech", 
            message: "Need a custom AI CRM platform for client operations.",
            project_type: "AI CRM Solutions", 
            budget: "$5,000 - $10,000", 
            status: "new",
            created_at: new Date().toISOString(),
            notes: "Initial request captured. High priority.",
            assignee: "Khalid"
          },
          { 
            id: "2", 
            ...
            company: "Dubai Retail", 
            message: "E-commerce platform upgrade and next-intl translation setup.",
          ...
          { 
            ...
            name: "Ali", 
            email: "ali@saas.io", 
            phone: "+965 99 555 1212",
            company: "SaaS Startups", 
            message: "Enterprise multi-tenant core platform with billing integration.",
            project_type: "SaaS Development", 
            budget: "$10,000+", 
            status: "proposal",
            created_at: new Date(Date.now() - 172800000).toISOString(),
            notes: "Sent initial technical architecture proposal.",
            assignee: "Sarah"
          }
        ])
      } else {
  ...
  return (
    <div className="space-y-8 h-full flex flex-col">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-3xl font-bold tracking-tight">CRM Leads Pipeline</h1>
          <p className="text-muted-foreground text-sm">Monitor business opportunities, pipeline stages, and client project proposals.</p>
        </div>
        <button 
          onClick={exportCSV}
      ...
      <div className="flex-1 overflow-x-auto pb-4 flex gap-6 items-start">
        {columns.map((col) => {
          ...
          return (
            <div key={col.id} className="w-80 flex-shrink-0 bg-card border border-border/60 rounded-xl p-4 flex flex-col max-h-[70vh]">
              ...
              <div className="space-y-3 overflow-y-auto flex-1 pr-1">
                {colLeads.map((lead) => (
                  <div 
                    ...
                    className="p-4 bg-secondary/35 border border-border/40 rounded-lg shadow-sm hover:border-primary/50 transition-all cursor-pointer space-y-3"
                  >
      ...
      {/* Leads Detail Modal Side-panel */}
      {selectedLead && (
        <div className="fixed inset-y-0 right-0 w-full max-w-md bg-card border-l border-border shadow-2xl z-40 flex flex-col p-8 transition-transform">
