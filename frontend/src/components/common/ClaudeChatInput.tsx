    ...
    return (
        <div className="relative group flex-shrink-0 w-28 h-28 rounded-2xl overflow-hidden border border-border bg-card animate-fade-in p-3 flex flex-col justify-between shadow-sm">
            ...
            <div className="flex items-center justify-between w-full mt-2">
                <div className="inline-flex items-center justify-center px-1.5 py-[2px] rounded border border-border bg-card dark:bg-transparent">
                    <span className="text-[9px] font-medium text-muted-foreground uppercase tracking-wider font-sans">PASTED</span>
                </div>
            </div>

            <button
                onClick={() => onRemove(content.id)}
                className="absolute top-2 right-2 p-[3px] bg-card dark:bg-muted border border-border rounded-full text-muted-foreground hover:text-foreground transition-colors shadow-sm opacity-0 group-hover:opacity-100"
            >
...
const ModelSelector: React.FC<ModelSelectorProps> = ({ models, selectedModel, onSelect }) => {
    ...
    return (
        <div className="relative" ref={dropdownRef}>
            ...
            {isOpen && (
                <div className="absolute bottom-full right-0 mb-2 w-[260px] bg-background border border-border/50 rounded-2xl shadow-2xl overflow-hidden z-50 flex flex-col p-1.5 animate-in fade-in origin-bottom-right">
                    {models.map(model => (
                        <button
                            key={model.id}
                            onClick={() => {
                            ...
                            className={`w-full text-left px-3 py-2.5 rounded-xl flex items-start justify-between group transition-colors hover:bg-muted/40`}
                        >
...
export interface ClaudeChatInputProps {
    onSendMessage: (data: {
        message: string;
        files: AttachedFileProps[];
        pastedContent: AttachedFileProps[];
        model: string;
...
export const ClaudeChatInput: React.FC<ClaudeChatInputProps> = ({ onSendMessage, disabled }) => {
    ...
    const [files, setFiles] = useState<AttachedFileProps[]>([]);
    const [pastedContent, setPastedContent] = useState<AttachedFileProps[]>([]);
    ...
    const handlePaste = (e: React.ClipboardEvent) => {
        ...
        if (text.length > 300) {
            ...
            const snippet = {
                id: Math.random().toString(36).substr(2, 9),
                file: new File([text], "pasted.txt", { type: "text/plain" }),
                type: "text/plain",
                preview: null,
    ...
    return (
        ...
        >
            ...
            <div className={`
                !box-content flex flex-col mx-2 md:mx-0 items-stretch transition-all duration-200 relative z-10 rounded-2xl cursor-text border border-border/50 
                shadow-[0_0_15px_rgba(0,0,0,0.08)] hover:shadow-[0_0_20px_rgba(0,0,0,0.12)]
                focus-within:shadow-[0_0_25px_rgba(0,0,0,0.15)]
                bg-background/80 backdrop-blur-xl font-sans antialiased
            `}>
                    ...
                    {(files.length > 0 || pastedContent.length > 0) && (
                        <div className="flex gap-3 overflow-x-auto custom-scrollbar pb-2 px-1">
                            {pastedContent.map(content => (
                                <PastedContentCard
                                    key={content.id}
                                    content={{ id: content.id, content: content.content || "", timestamp: new Date() }}
                        ...
                        <div className="relative flex-1 flex items-center shrink min-w-0 gap-1">
                            ...
                            <div className="flex shrink min-w-8 !shrink-0 group">
                                ...
                                >
                                    ...
                                    <div className="absolute top-full left-1/2 -translate-x-1/2 mt-2 px-2 py-1 bg-foreground text-background text-[11px] font-medium rounded-[6px] opacity-0 group-hover:opacity-100 transition-opacity whitespace-nowrap pointer-events-none z-50 flex items-center gap-1 shadow-sm tracking-wide">
            ...
            {
                isDragging && (
                    <div className="absolute inset-0 bg-background/90 border-2 border-dashed border-primary rounded-2xl z-50 flex flex-col items-center justify-center backdrop-blur-sm pointer-events-none">
