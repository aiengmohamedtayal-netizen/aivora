...
const navItems = [
  ...
  { name: "Projects & Tasks", href: "/admin/projects", icon: CheckSquare },
  ...
  { name: "AI Console", href: "/admin/ai-console", icon: Bot },
...
export function Sidebar() {
  ...
  return (
    <aside className="w-64 border-r border-border bg-card h-screen fixed left-0 top-0 flex flex-col pt-8 z-20">
      <div className="px-6 mb-8 flex flex-col gap-2">
        <Link href="/" className="flex items-center text-sm text-muted-foreground hover:text-foreground transition-colors mb-4">
          <ArrowLeft className="w-4 h-4 mr-2 rtl:rotate-180" /> Back to website
        </Link>
        <h1 className="text-2xl font-bold tracking-tight text-primary">Aivora OS</h1>
        <span className="text-[10px] text-muted-foreground uppercase tracking-widest font-semibold">Business Operations</span>
      </div>
      ...
      <div className="p-4 border-t border-border/50">
        <button 
          onClick={async () => {
            // Signs out by redirecting to a helper endpoint or clearing session
